...
bool within_bounds(struct options *opts, short x, short y)
{
    if (    opts->width/2  > abs(x)
         && opts->height/2 > abs(y)
       ) return true;
    else return false;
}
...
void spiral(SDL_Surface *screen, struct options *opts, double r, double T)
{
  double t;
  short x, y;
  for (t=T; t>=T-28; t--)
  {
    x = ( 6.5*r*cos(r-t/10.0) 
    +  90.0*cos(t/100.0+sqrt(r));
    +  90.0*cos(t/20.0) );
    y = 6.5*r*sin(r-t/10.0);
    if (within_bounds(opts, x, y))
    {
       putPixel(screen, x+opts->width/2, y+opts->height/2, 0x00FFFFFF - 0x00070707*(T-t));
    }
  }
}
...
int main(int argc, char **argv) {
 ...
 while (t<opts->seconds)
 {
   ...
   for (i=0; i<opts->spirals; i++)
   {

      #pragma omp parallel for num_threads(4)
      for (i=opts->width; i<bound; i++) 
      {
        r = (float)i/128.0;
        spiral(screen, opts, r, t*opts->framerate);
      }
